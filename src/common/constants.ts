export const LOADERTEXT = {
    DEFAULT: "Loading data",
    INITIAL_LOAD: "Initial loading may take up to 30 to 50 seconds due to limitations of free-tier hosting. Thank you for your patience.",
    REGISTER: "Registering your new account.",
    SIGN_IN: "Signing you in.",
    ORDER_PLACEMENT: "Processing your order request.",
};

export const SUCCESSMESSAGE = {
    PRODUCT_UPDATE: { TITLE: "Product Updated Successfully", SUBTITLE: 'Your product has been updated successfully. You can now see the changes.' },
};

export const CONFIRMATIONMESSAGE = {
    PRODUCT_DELETE: "Are you sure you want to delete this product? This action cannot be undone.",
};

